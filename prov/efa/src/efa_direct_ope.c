...
int efa_direct_ope_pool_create(struct efa_ep *ep)
{
	...
	if (!efa_env.track_mr) {
		ep->direct_ope_pool = NULL;
		return 0;
	}

	dlist_init(&ep->direct_ope_list);

	ret = ofi_bufpool_create(&ep->direct_ope_pool,
	...
	ret = ofi_bufpool_grow(ep->direct_ope_pool);
	if (ret) {
		ofi_bufpool_destroy(ep->direct_ope_pool);
		ep->direct_ope_pool = NULL;
		return ret;
	}
...
void efa_direct_ope_pool_destroy(struct efa_ep *ep)
{
	...
	if (!ep->direct_ope_pool)
		return;

	ofi_genlock_lock(&ep->base_ep.domain->util_domain.lock);
	if (!dlist_empty(&ep->direct_ope_list)) {
		dlist_foreach_container_safe(&ep->direct_ope_list,
...
{
	struct efa_ep *ep = container_of(base_ep, struct efa_ep, base_ep);
	struct efa_direct_ope *direct_ope;
				  ...
				  struct efa_direct_ope *direct_ope)
{
	...
	if (!direct_ope || !ep->direct_ope_pool)
		return;
