						...
						struct ibv_cq_ex *rx_cq)
{
	...
	attr_ex->cap.max_inline_data = info->tx_attr->inject_size;
			  ...
			  void *context)
{
	...
	/* Set inject sizes based on info->tx_attr->inject_size */
	if (base_ep->info->tx_attr->inject_size <= base_ep->domain->device->efa_attr.inline_buf_size) {
		/* Regular WQE: only MSG inject supported */
		base_ep->inject_msg_size = base_ep->info->tx_attr->inject_size;
		base_ep->inject_rma_size = 0;
	} else {
		/* Wide WQE: both MSG and RMA inject supported */
		base_ep->inject_msg_size = base_ep->info->tx_attr->inject_size;
		base_ep->inject_rma_size = base_ep->info->tx_attr->inject_size;
	}
