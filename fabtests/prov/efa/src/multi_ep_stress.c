...
static void *run_sender_worker(void *arg)
{
	...
	char ep_addr[MAX_EP_ADDR_LEN][ctx->num_peers];
	...
	for (size_t i = 0; i < ctx->num_peers; i++) {
		...
		assert(msg.info.peer_idx == i);
		memcpy(ep_addr[i], msg.info.ep_addr, MAX_EP_ADDR_LEN);
		memcpy(&peer_rma[i], &msg.info.rma, sizeof(struct rma_info));
	...
	while(ops_posted < total_ops) {
		if (ops_posted_in_this_cycle == ops_total_in_this_cycle) {
			...
			ret = fi_av_insert(ctx->av, ep_addr, ctx->num_peers, fi_addr, 0, NULL);
			if (ret != ctx->num_peers) {
...
static int notify_endpoint_update(struct worker_context *ctx, void *rma_buffer)
{
	...
	for (size_t i = 0; i < ctx->num_peers; i++) {
		...
		msg.info.peer_idx = i / topts.sender_ep_recycling;
